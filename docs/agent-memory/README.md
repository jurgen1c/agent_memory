...
Use `bin/memory new claim` for a safe low-confidence claim draft and `bin/memory new recipe` for a first-class reusable procedure draft. Both start in `needs_review`; replace all TODO values and complete verification before promotion to `current`. Record the tested Git commit in `last_verified_commit` when a claim is verified.
